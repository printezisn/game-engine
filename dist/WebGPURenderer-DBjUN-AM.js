import { E as l, g as w, f as ne, D as P, S as T, B as S, d as oe, h as R, i as v, w as E, j as b, k as ae, T as ue, l as A, m as U, M as k, o as L, p as ce, q as de, s as he, t as D, u as H, a as M, e as G, A as pe, R as le } from "./index-IMbOw1ZD.js";
import { S as z, l as fe, a as ge } from "./colorToUniform-BB0HiXFj.js";
import { c as me, u as _e, U as be, B as ye, G as xe, e as Ge, R as Be, t as Se, S as Pe, a as Te } from "./SharedSystems-Dc-FjMd5.js";
const y = z.for2d();
class F {
  start(e, t, r) {
  ...
  _createBindGroup(e, t, r) {
    ...
    for (const f in i) {
      ...
      if (h._resourceType === "uniformGroup") {
        ...
        const d = h;
        p = o.texture.getGpuSource(d).createView({});
      }
      n.push({
        binding: i[f],
        resource: p
...
function ve(a) {
  ...
  for (let r = 0; r < e.length; r++) {
    const s = e[r];
    let i = x[s.data.type].size;
    const n = x[s.data.type].align;
    if (!x[s.data.type])
...
function Ue(a, e) {
  const { size: t, align: r } = x[a.data.type], s = (r - t) / 4;
  return `
         v = uv.${a.data.name};
         ${e !== 0 ? `offset += ${e};` : ""}
  ...
  _createVertexBufferLayouts(e, t) {
    ...
    return e.buffers.forEach((i) => {
      ...
      for (const u in t.attributeData) {
        ...
        (c.divisor ?? 1) !== 1 && E(`Attribute ${u} has an invalid divisor value of '${c.divisor}'. WebGPU only supports a divisor value of 1`), c.buffer === i && (n.arrayStride = c.stride, n.stepMode = c.instance ? "instance" : "vertex", o.push({
...
class ke {
  ...
  getDescriptor(e, t, r) {
    ...
    const s = this._renderTargetSystem, i = s.getGpuRenderTarget(e), n = e.colorTextures.map(
      (c, f) => {
        const h = i.contexts[f];
        let p, d;
        h ? p = h.getCurrentTexture().createView() : p = this._renderer.texture.getGpuSource(c).createView({
          mipLevelCount: 1
        }), i.msaaTextures[f] && (d = p, p = this._renderer.texture.getTextureView(
          i.msaaTextures[f]
        ));
        const g = t & b.COLOR ? "clear" : "load";
    ...
    if ((e.stencil || e.depth) && !e.depthStencilTexture && (e.ensureDepthStencilTexture(), e.depthStencilTexture.source.sampleCount = i.msaa ? 4 : 1), e.depthStencilTexture) {
      const c = t & b.STENCIL ? "clear" : "load", f = t & b.DEPTH ? "clear" : "load";
      o = {
  ...
  clear(e, t = !0, r, s) {
    ...
    if (n.commandEncoder === null) {
      const c = o.createCommandEncoder(), f = this.getDescriptor(e, t, r), h = c.beginRenderPass(f);
...
class X {
  constructor() {
    this.defaultState = new z(), this.defaultState.blend = !0;
  }
  contextChange(e) {
    this.gpu = e;
  }
...
class Q {
  ...
  _createTextureBindGroup(e) {
    const t = e.source;
    return this._bindGroupHash[e.uid] = new R({
      0: t,
      1: t.style,
      2: new A({
  ...
  getPixels(e) {
    const t = this.generateCanvas(e), r = U.getOptimalCanvasAndContext(t.width, t.height), s = r.context;
    s.drawImage(t, 0, 0);
    const { width: i, height: n } = t, o = s.getImageData(0, 0, i, n), u = new Uint8ClampedArray(o.data.buffer);
    return U.returnCanvasAndContext(r), { pixels: u, width: i, height: n };
  }
  destroy() {
...
class ee {
  ...
  execute(e, t) {
    ...
    for (let d = 0; d < u.instructionSize; d++) {
      const g = p[d];
      if (s.groups[1] = g.bindGroup, !g.gpuBindGroup) {
        const B = g.textures;
        g.bindGroup = w(B.textures, B.count), g.gpuBindGroup = i.bindGroup.getBindGroup(
          g.bindGroup,
          s.gpuProgram,
          1
        );
      }
      c.setBindGroup(1, g.bindGroup, s.gpuProgram), c.renderPassEncoder.drawIndexed(g.size, 1, g.start);
    }
  }
  destroy() {
...
class te {
  ...
  execute(e, t) {
    ...
    if (!s)
      ...
      E("Mesh shader has no gpuProgram", t.shader);
      return;
    }
