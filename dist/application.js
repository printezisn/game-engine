...
const resizeCanvas = () => {
    const { width, height, orientation } = resize(config.gameContainer, app.canvas, config.screen.width, getScreenHeight());
...
const handleTick = () => {
    ...
    if (config.debug) {
        setInterval(() => {
            console.log(totalFPSEntries === 0 ? 0 : Math.floor(totalFPS / totalFPSEntries));
            totalFPSEntries = 0;
            totalFPS = 0;
        }, 1000);
    }
...
export const changeScene = async (newScene) => {
    if (gameState.scene) {
...
export const initGame = async () => {
    ...
    handleTick();
    await Promise.all([
        new Promise((resolve) => setTimeout(resolve, config.loadingScene.keepAliveTimeMS)),
        (async () => {
            await Assets.init({
