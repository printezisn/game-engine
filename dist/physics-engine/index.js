...
export const initPhysicsEngine = () => {
    ...
    Events.on(engine, 'collisionStart', (e) => {
        e.pairs.forEach((pair) => {
            ...
            if (!nonSurface)
                return;
            if (surface) {
                touchingGround.set(nonSurface.target.matterBody.label, Math.floor(getBounds(surface).y1));
            }
            else {
    ...
    Events.on(engine, 'afterUpdate', () => {
        entitiesToUpdate.forEach((entity) => {
            if (!entity.target.matterBody)
                return;
            const bounds = getBounds(entity);
            const onGround = (touchingGround.get(entity.target.matterBody.label) ?? -Infinity) >=
                Math.floor(bounds.y2);
            entity.onUpdatePosition?.(bounds.x1, bounds.y1, onGround);
        });
    });
};
export const updatePhysics = (interval) => {
    Engine.update(engine, interval);
};
export const addPhysicalEntity = (entity) => {
    if (entity.rectangle) {
        entity.target.matterBody = Bodies.rectangle(entity.rectangle.x + entity.rectangle.width / 2, entity.rectangle.y + entity.rectangle.height / 2, entity.rectangle.width, entity.rectangle.height, createBodyDefinitionOptions(entity));
    }
    else if (entity.circle) {
        entity.target.matterBody = Bodies.circle(entity.circle.x, entity.circle.y, entity.circle.radius, createBodyDefinitionOptions(entity));
    }
    else {
...
export const removePhysicalEntity = (target) => {
    if (!target.matterBody)
        return;
...
export const setMovement = (target, movement) => {
    if (!target.matterBody)
        return;
...
export const movePhysicalEntity = (target, x, y) => {
    if (!target.matterBody)
        return;
...
const createBodyDefinitionOptions = (entity) => {
    entityLabel++;
    if (entity.surface) {
        return {
            isStatic: true,
